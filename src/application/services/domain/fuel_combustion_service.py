...
def create_fuel_combustion_entity(type: ActivityType, source_type_name: str, consumption_value: Decimal) -> FuelCombustion:
    try:    
        if not isinstance(type: ActivityType) or not isinstance(source_type_name: str) or not isinstance(consumption_value: Decimal):
            raise TypeError()
